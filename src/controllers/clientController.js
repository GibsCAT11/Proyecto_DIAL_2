import Product from '../entities/Client.js';
import ProductDAO from '../daos/ClientDAO.js';
import Client from '../entities/Client.js';

export const create = async (req, res) => {
    try {
        const data = new Client(null, req.body.name, req.body.phone, req.body.email, req.body.adress);
        const clientProduct = await ClientDAO.create(data);
        return res.status(201).json(clientProduct);
...
export const getAll = async (req, res) => {
    try {
        const client = await ClientDAO.getAll();
